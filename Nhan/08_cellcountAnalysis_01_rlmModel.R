...
# run the rlm() model ------------------------------------------------
cellcounts <- names(cohortDat$allSample$cellcount)
...
inputDat_discovery <- cohortDat$donor_info %>%
  ...
  inner_join(cohortDat$Discovery$cellcount %>% rownames_to_column("Record.Id")) %>%
...
for (cellcount in cellcounts) {
  dat_temp <- inputDat_discovery %>% 
    dplyr::rename("valName" = cellcount) %>% 
  ...
  rlm_res_discovery[[cellcount]] <- cf["CMV_IgG_Serology1", c("Estimate", "Std. Error", "z value", "Pr(>|z|)")]
...
inputDat_validation <- cohortDat$donor_info %>%
  ...
  inner_join(cohortDat$Validation$cellcount %>% rownames_to_column("Record.Id")) %>%
  mutate(SEX_BIRTH = as.factor(SEX_BIRTH), 
         CMV_IgG_Serology = as.factor(CMV_IgG_Serology))

sigCellcounts_discovery <- rownames(outcome_discovery %>% filter(padj < 0.05))

rlm_res_validation <- list()
for (cellcount in sigCellcounts_discovery) {
  dat_temp <- inputDat_validation %>% 
    dplyr::rename("valName" = cellcount) %>% 
    mutate(valName = inormal(valName)) # inverse ranking transformation to have normal distribution
  
  ttest <- rlm(valName ~ CMV_IgG_Serology + AGE + BMI_BASELINE #+ 
                # SEX_BIRTH + season_sin + season_cos + PC1 + PC2 + PC3 + PC4 +PC5
               , 
               data = dat_temp, maxit=200)
  cf <- try(coeftest(ttest, vcov=vcovHC(ttest, type="HC0")))
  rlm_res_validation[[cellcount]] <- cf["CMV_IgG_Serology1", c("Estimate", "Std. Error", "z value", "Pr(>|z|)")]
}
...
save(rlm_res, file = "/vol/projects/BIIM/cohorts_CMV/CMV_2000HIV_NhanNguyen/processedDat/rlmRes_cellcount.RData")
